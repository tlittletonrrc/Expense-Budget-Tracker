...
---
The allocation repository is used to fetch, add, update, and delete any data related to allocations. The use of this repository separates concerns by separating the validation and retrieval functions into their own layers.
...
**getUserByIDService:** This service grabs a specific user using their ID from the repo. if the checks pass it returns the specific user. This is used in the AllocationRepository to grab and manipulate allocation data on specific users.